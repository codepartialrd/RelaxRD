...
class NaiveCore:
    ...
    def naivecore(self):
        ...
        S = []
        t1=time.time()
        for r in range(1, len(self.sigma) + 1):
            for combo in combinations(self.sigma, r):
                if self._is_redundancy(combo):
                    continue
                S.append(combo)
        print(f"[Effectiveness] Number of candidate AFD subsets after NaiveCore, |S_NaiveCore|: {len(S)}")
        print('-------------------------------------------------------------------------------------')
        print(f"[Efficiency] Time of filtering using NaiveCore_1, Σ_NaiveCore_1: {tf-tw:.4f} seconds")

...
if __name__ == "__main__":
    ...
    t1 = time.time()
    file_path = '../dataset/fraud/fraudTrain.csv'
    file_name = file_path.split('/')[-1].split('.')[0]
    data_pd, T,r,columns=read_db_from_csv(file_path, sep=',')
    error = 0.05
    taneappr = TANE(T)
    t0 = time.time()
    taneappr.run(r, error)
    print("\t=> Execution Time: {} seconds".format(time.time() - t0))
    print('\t=> {} Rules Found'.format(len(taneappr.rules)))

    new_rules = []
    new_error_dict = {}
    i=0
    for lhs, rhs in taneappr.rules:
        lhs_names = tuple(columns[i] for i in lhs)
        rhs_name = columns[rhs]
        i+=1
        print(lhs_names, rhs_name)
        new_rules.append((lhs_names, rhs_name))
        if (lhs, rhs) in taneappr.error_dict:
            new_error_dict[(lhs_names, rhs_name)] = taneappr.error_dict[(lhs, rhs)]
    print(i)
    with open(
            "afd_pkl/fraudtrain.pkl",
            "wb") as f:
        pkl.dump({
            "fds_dict_list": new_rules,
